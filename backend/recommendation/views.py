...
class TF_IDF_View(APIView):
    def get(self, request, user_id):
        ...
        orders = {
            '10000080': {'user': '00000000003', 'menus': ['파송송 라면', '불닭 국수', '닭가슴살 샐러드']},
            '10000082': {'user': '00000000003', 'menus': ['쌀국수', '불닭볶음면', '소고기 샐러드']}
        }
        ...
        # 사용자의 과거 주문에서 메뉴 추출
        past_orders = [order['menus'] for order in orders.values() if order['user'] == user_id]
        past_menus = [menu for order in past_orders for menu in order]
        ...
        recommended_menus = [menu for menu in sorted_menus if menu not in past_menus]
